...
import { getPlayerPositionSortValue } from "../simulator/helpers";
...
export function playerSorterByPowerReverse(playerA: IPlayer, playerB: IPlayer) {
  if (playerA.power === playerB.power) {
    return playerSorter(playerA, playerB);
  }
  return playerA.power < playerB.power ? -1 : 1;
...
export function playerSorterByDeffensivePower(playerA: IPlayer, playerB: IPlayer) {
  if (playerA.position !== playerB.position) {
    return playerSorter(playerA, playerB);
  }
  return playerSorterByPower(playerA, playerB);
